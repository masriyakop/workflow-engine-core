...
final class WorkflowInstance
{
    ...
    /**
     * Remove a step from the completed list so it can run again (kemaskini re-entry).
     */
    public function removeCompletedStep(string $stepId): void
    {
        $this->completedSteps = array_values(array_filter(
            $this->completedSteps,
            static fn (string $id): bool => $id !== $stepId
        ));
        $this->updatedAt = new \DateTime;
    }
