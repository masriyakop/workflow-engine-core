...
class Executor
{
    ...
    private function processWorkflow(WorkflowInstance $instance): void
    {
        ...
        // Resume / retry the current incomplete step first (human wait gates)
        $currentStepId = $instance->getCurrentStepId();
        if ($currentStepId !== null && ! $instance->isStepCompleted($currentStepId)) {
            $currentStep = $instance->getDefinition()->getStep($currentStepId);
            if ($currentStep && $instance->canExecuteStep($currentStepId)) {
        ...
        foreach ($nextSteps as $step) {
            if ($instance->isStepCompleted($step->getId())) {
                ...
                if ($step->getActionClass() === HumanTaskAction::class
                    ...
                    unset($humanTasks[$step->getId()]);
                    $instance->mergeData(['_human_tasks' => $humanTasks]);
                    $this->stateManager->save($instance);
                } else {
                    continue; // Skip already completed non-human steps
                }
            }
            ...
            // Stop the foreach if we entered WAITING mid-loop
    ...
    private function executeStep(WorkflowInstance $instance, Step $step): void
    {
        ...
        try {
            if ($step->hasAction()) {
                $waiting = $this->executeActionWithRetry($instance, $step);
                if ($waiting) {
                    return;
                }
            }
     ...
     * @param WorkflowInstance $instance The workflow instance
     * @param Step $step The step to execute
     * @return bool True when the action entered WAITING (step not completed)
     *
     * @throws ActionNotFoundException If the action class doesn't exist
     * @throws StepExecutionException If all retry attempts are exhausted
     */
    private function executeActionWithRetry(WorkflowInstance $instance, Step $step): bool
    {
        ...
        throw $lastException ?? new \RuntimeException('Action retry loop exited unexpectedly');
    }
     ...
     */
    private function executeAction(WorkflowInstance $instance, Step $step): bool
    {
        ...
        if ($result->isWaiting()) {
            ...
            $this->stateManager->save($instance);
            $this->eventDispatcher->dispatch(new WorkflowWaitingEvent($instance, $step));

            $this->logger->info('Workflow step waiting for external input', [
                'workflow_id' => $instance->getId(),
                'step_id' => $step->getId(),
