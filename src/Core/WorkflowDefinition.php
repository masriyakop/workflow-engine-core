...
final class WorkflowDefinition
{
    ...
    public function getNextSteps(?string $currentStepId, array $data = []): array
    {
        ...
        // Sequential fallback when no explicit transitions are defined
        if ($nextSteps === [] && $this->transitions === []) {
            $stepIds = array_keys($this->steps);
            $index = array_search($currentStepId, $stepIds, true);
            if ($index !== false && isset($stepIds[$index + 1])) {
                $next = $this->getStep($stepIds[$index + 1]);
                if ($next) {
                    $nextSteps[] = $next;
                }
            }
